...
app.get("/", (req, res) => {
  if(req.session.user_id===undefined){
    ...
    res.render("partials/_header.ejs",{user_id:req.session.user_id});
  }
});
...
app.post("/urls", (req, res) => {
  ...
  urlDatabase[shortURL]={'longURL':req.body['longURL'],'user_id':req.session.user_id,'shortURL':shortURL};
  console.log(urlDatabase[shortURL]);
  let templateVars = { user_id: req.session.user_id, urls: listLinks(req.session.user_id)};
...
app.post("/login", (req, res) => {
  ...
  let loggedin = false;
  for(var x in users){
    if(users[x]['email'] === req.body['user_id'] && bcrypt.compareSync(req.body['password']),users[x]['password']){
  ...
  if(loggedin){
    //console.log(req.session.user_id+ " is the cookie value!");
    //console.log(req.body)
...
app.get("/urls/:id", (req, res) => {
  ...
  if(urlDatabase.hasOwnProperty(id)){
    if(urlDatabase[id]['user_id']===req.session.user_id){
    
    templateVars = { user_id: req.session.user_id,response : 'Full URL:' +urlDatabase[id]['longURL']+"  Short form:"+id, shorturl:id,urlfound:true};
    }else{
     templateVars = { user_id: req.session.user_id,response :'shorturl not found',urlfound:false}
    }
  }
